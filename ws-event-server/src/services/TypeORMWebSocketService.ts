...
export class TypeORMWebSocketService extends EventEmitter {
  private clients = new Map<WebSocket, ConnectedClient>();
  private sessionService: TypeORMSessionService;
  private heartbeatInterval?: NodeJS.Timeout;

  constructor(sessionService: TypeORMSessionService) {
    super();
  ...
  private async handleMessage(ws: WebSocket, data: any): Promise<void> {
    try {
      ...
      switch (message.type) {
        case "session_start":
          await this.handleSessionStart(ws, message.data);
          break;

        case "events_batch":
          await this.handleEventsBatch(ws, message.data);
          break;

        case "session_end":
          await this.handleSessionEnd(message.data.sessionId);
          break;

        case "viewer_join_session":
          await this.handleViewerJoinSession(ws, message.data.sessionId);
          break;
        ...
        case "get_active_sessions":
          await this.sendActiveSessions(ws);
          break;

        case "get_session_events":
          await this.handleGetSessionEvents(
            ws,
        ...
        case "error":
          await this.handleClientError(ws, message.data);
          break;

        case "visibility_change":
        case "javascript_error":
        case "promise_rejection":
          await this.handleSessionEvent(ws, message);
          break;
  ...
  private async handleSessionStart(ws: WebSocket, data: any): Promise<void> {
    const client = this.clients.get(ws);
  ...
  private async handleEventsBatch(ws: WebSocket, data: any): Promise<void> {
    const client = this.clients.get(ws);
    if (!client || client.type !== "tracker" || !client.sessionId) return;

    const { events } = data;
    await this.sessionService.addEventsToSession(client.sessionId, events);
  }
  ...
  private async handleViewerJoinSession(
    ws: WebSocket,
    sessionId: string
  ): Promise<void> {
    const client = this.clients.get(ws);
    ...
    if (session) {
      ...
      // If session not in memory, ask DB for events to infer counts
  ...
  private async handleGetSessionEvents(
    ws: WebSocket,
    sessionId: string,
    fromIndex = 0
  ): Promise<void> {
  ...
  private async handleClientError(
    ...
    await this.sessionService.addErrorToSession(client.sessionId, errorData);
  }

  private async handleSessionEvent(ws: WebSocket, message: any): Promise<void> {
    const client = this.clients.get(ws);
    if (!client || !client.sessionId) return;
  ...
  private async sendActiveSessions(ws: WebSocket): Promise<void> {
    const activeSessions = await this.sessionService.getActiveSessionsFromDB();
...
export default TypeORMWebSocketService;
